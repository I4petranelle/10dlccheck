...
(function aiEmailGate() {
  ...
  if (results) {
    ...
    onResultsRender();
    new MutationObserver(onResultsRender).observe(results, { childList: true, subtree: true });
  }

  if (aiOptIn) {
    aiOptIn.addEventListener("change", () => {
      if (!aiOptIn.checked) { hideAiCompletely(); return; }
      const sessionEmail = sessionStorage.getItem(SESSION_KEY);
      if (sessionEmail) {
        if (aiLeadPanel) aiLeadPanel.style.display = "none";
        if (aiPanel)     aiPanel.style.display     = "block";
  ...
  async function renderAiSuggestion() {
    ...
    if (!aiOptIn || !aiOptIn.checked || !message) { resetAiSuggestionBox(); return; }
    ...
    try {
      ...
      if (ct.includes("application/json")) {
        ...
        data = (txt && txt.trim().startsWith("{")) ? JSON.parse(txt) : { suggestion: "", risks: [] };
      }

      if (aiText)  aiText.textContent  = (data?.suggestion || "").trim() || "No suggestion returned.";
      if (aiFlags) aiFlags.textContent = Array.isArray(data?.risks) && data.risks.length
...
// =========================
// Export bar — delayed 2.5s after results appear
// =========================
(function setupExportBar() {
  ...
  emailBtn?.addEventListener('click', () => {
    alert('Email sending is coming soon. Please copy the report instead.');
  });
})();
...
(function () {
  ...
  const ucGuidance= document.getElementById('ucGuidance');
  const ucAutoTag = document.getElementById('ucAutoTag');
  ...
  clearBtn.addEventListener('click', () => {
    ...
    // Also reset use case selector
    if (ucSelect)   ucSelect.value = "";
    if (ucGuidance) { ucGuidance.className = ''; ucGuidance.innerHTML = ''; }
    if (ucAutoTag)  ucAutoTag.style.display = "none";
    if (msg)        msg.focus();
  });
})();